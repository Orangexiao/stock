      ...
      return {
        post: function ($scope, $elm, $attrs, uiGridCtrl) {
          ...
          function gridResize($event) {
             ...
             * 修改内容：控制GRID最大化
             */
            //if(gridUtil.elementHeight($elm)===0){
            //  grid.gridHeight = $scope.gridHeight;
            //  height = (($window.innerHeight > 0) ? $window.innerHeight : $window.screen.height) - 1;
            //  height = height - topOffset;
            //  grid.gridHeight = $scope.gridHeight = height;
  ...
  ['width', 'height'].forEach(function (name) {
    ...
    s['element' + capsName] = function (elem, extra) {
      ...
      if (e) {
        ...
        eleSize = e.offsetWidth === 0 && rdisplayswap.test(styles.display) ?
  ...
  module.service('uiGridPaginationService', ['gridUtil',
    function (gridUtil) {
      var service = {
         ...
         * @param {int} pageSize requested page size
         */
        onPaginationChanged: function (grid, currentPage, pageSize,pageObj) {
            grid.api.pagination.raise.paginationChanged(currentPage, pageSize,pageObj);
      ...
      return {
        ...
        link: function ($scope, $elm, $attr, uiGridCtrl) {
          ...
          var deregP = $scope.$watch('grid.options.paginationCurrentPage + grid.options.paginationPageSize', function (newValues, oldValues) {
              ...
              setShowing();
                var pageObj = {
                  'currentPage' : options.paginationCurrentPage,
                  'pageSize' : options.paginationPageSize,
                  'firstRow' : (options.paginationCurrentPage-1)*options.paginationPageSize,
                  'lastRow' : options.paginationCurrentPage*options.paginationPageSize
                }
                uiGridPaginationService.onPaginationChanged($scope.grid, options.paginationCurrentPage, options.paginationPageSize,pageObj)
            }
          );
